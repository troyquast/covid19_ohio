...
#delimit ;
log using "covid-19 ohio state nat causes by gender analysis - 02 - estimation - v7 .smcl", replace ;

* open sample data ;
	use "covid-19 ohio state nat causes by gender analysis - sample data - v6 .dta", clear ;
...
* merge w/ full dataset ;
	merge 1:1 year_cal week_mmwr gender using "covid-19 ohio state nat causes by gender analysis - sample data - v6 .dta" ;
* prep & save data ;
	...
	* rename vars ;
		foreach G in female male
			{ ;
			rename num_deaths`G' st_natc_`G'_actual ;
			foreach P in 005 025 050 500 950 975 995 
				{ ;
				rename pctile_`P'`G' st_natc_`G'_p`P' ;
				} ;
			} ;
	* order & sort data ;
		gsort year_cal week_mmwr ;
		order year_cal week_mmwr year_epi st_natc_female_actual st_natc_female_p* st_natc_male_actual st_natc_male_p* ;
	* save data ;
		save "covid-19 ohio state nat causes by gender analysis - actual & counter counts - v7 .dta", replace ;
log close ;
end ;