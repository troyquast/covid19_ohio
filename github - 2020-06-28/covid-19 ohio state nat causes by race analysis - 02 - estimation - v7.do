...
#delimit ;
log using "covid-19 ohio state nat causes by race analysis - 02 - estimation - v7 .smcl", replace ;

* open sample data ;
	use "covid-19 ohio state nat causes by race analysis - sample data - v6 .dta", clear ;
...
* merge w/ full dataset ;
	merge 1:1 year_cal week_mmwr race using "covid-19 ohio state nat causes by race analysis - sample data - v6 .dta" ;
* prep & save data ;
	...
	* rename vars ;
		foreach R in black white
			{ ;
			rename num_deaths`R' st_natc_`R'_actual ;
			foreach P in 005 025 050 500 950 975 995 
				{ ;
				rename pctile_`P'`R' st_natc_`R'_p`P' ;
				} ;
			} ;
	* order & sort data ;
		gsort year_cal week_mmwr ;
		order year_cal week_mmwr year_epi st_natc_black_actual st_natc_black_p* st_natc_white_actual st_natc_white_p* ;
	* save data ;
		save "covid-19 ohio state nat causes by race analysis - actual & counter counts - v7 .dta", replace ;
log close ;
end ;