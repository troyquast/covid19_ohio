...
#delimit ;
log using "covid-19 ohio county nat causes analysis - 02 - estimation - v7 .smcl", replace ;

* open sample data ;
	use "covid-19 ohio county nat causes analysis - sample data - v6 .dta", clear ;
...
* merge w/ full dataset ;
	merge 1:1 county year_cal week_mmwr using "covid-19 ohio county nat causes analysis - sample data - v6 .dta" ;
* prep & save data ;
	...
	* rename vars ;
		rename num_deaths cty_natc_actual ;
		foreach P in 005 025 050 500 950 975 995 
			{ ;
			rename pctile_`P' cty_natc_p`P' ;
			} ;
	* order & sort data ;
		gsort county year_cal week_mmwr ;
		order county year_cal week_mmwr year_epi cty_natc_* ;
	* save data ;
		save "covid-19 ohio county nat causes analysis - actual & counter counts - v7 .dta", replace ;
log close ;
end ;