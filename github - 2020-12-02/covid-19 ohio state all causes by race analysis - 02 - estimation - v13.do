...
#delimit ;
log using "C:\Users\Troy\Box Sync\_ troy's files\research\covid-19\2020-05 - ohio analysis\stata work\covid-19 ohio state all causes by race analysis - 02 - estimation - v13 .smcl", replace ;

* open sample data ;
	use "C:\Users\Troy\Box Sync\_ troy's files\research\covid-19\2020-05 - ohio analysis\stata work\covid-19 ohio state all causes by race analysis - sample data - v13 .dta", clear ;
	...
	* using fourier terms ;
		foreach R in black white
			...
			foreach W of numlist 1/40
				...
				* create scalar that is the pop in 2020 for that race (to be used in prediction calculation below) ;
					sum pop_total if race == "`R'" & year_cal == "2020" ;
		...
		* append race-week files ;
			* start w/ blacks ;
				use "`black_1'" ;
				foreach W of numlist 2/40
					{ ;
					append using "`black_`W''" ;
					} ;
			* append white observations ;
				foreach W of numlist 1/40
					{ ;
					append using "`white_`W''" ;
					} ;
		* add year vars ;
			gen year_cal = "2020" ;
			gen year_epi = "2019" ;
		* sort data ;
			gsort year_cal week_mmwr race ;
* merge w/ full dataset ;
	merge 1:1 year_cal week_mmwr race using "C:\Users\Troy\Box Sync\_ troy's files\research\covid-19\2020-05 - ohio analysis\stata work\covid-19 ohio state all causes by race analysis - sample data - v13 .dta" ;
* prep & save data ;
	* limit to necessary vars ;
		keep race year_cal year_epi week_mmwr num_deaths pctile_005 pctile_025 pctile_050 pctile_500 pctile_950 pctile_975 pctile_995 ;
	...
	* reshape to wide format ;
		reshape wide 
			...
			i(year_cal week_mmwr year_epi) j(race) string ;
	* rename vars ;
		foreach R in black white
			{ ;
	...
	* order & sort data ;
		...
		order year_cal week_mmwr year_epi st_allc_black_actual st_allc_black_p* st_allc_white_actual st_allc_white_p* ;
	* save data ;
		save "C:\Users\Troy\Box Sync\_ troy's files\research\covid-19\2020-05 - ohio analysis\stata work\counter mort datasets\covid-19 ohio state all causes by race analysis - actual & counter counts - v13 .dta", replace ;
log close ;
end ;