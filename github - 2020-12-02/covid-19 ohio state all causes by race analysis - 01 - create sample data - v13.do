...
log using "C:\Users\Troy\Box Sync\_ troy's files\research\covid-19\2020-05 - ohio analysis\stata work\covid-19 ohio state all causes by race analysis - 01 - create sample data - v13 .smcl", replace ;
* read in, revise, & save datasets ;
	* deaths data ;
		* read in data ;
			import excel 
				...
				sheet("dths- st all race") firstrow allstring ;
		* drop total row ;
			drop if DeathYearYear == "Total" ;
			drop if DeathRaceRaceDesc == "Total" ;
		...
		* rename vars ;
			rename 
				(DeathRaceRaceDesc DeathYearYear MMWRWeekMMWRWeek Deaths     )
				(race              year_cal      week_mmwr        num_deaths ) ;
		...
		* inspect var values ;
			* # deaths ;
				...
				* by race ;
					gsort race ;
					tabstat num_deaths, stat(sum) format(%9.0fc) by(race) ;
					drop if race == "American Indian or Alaskan Native" ;
					drop if race == "Asian or Pacific Islander" ;
					drop if race == "Other" ;
					drop if race == "Unknown" ;
					tabstat num_deaths, stat(sum) format(%9.0fc) by(race) ;
		* revise race values to lower case ;
	...
	* pop data ;
		* read in data ;
			import excel 
				"C:\Users\Troy\Box Sync\_ troy's files\research\_data\ohio\public health info warehouse\ohio deaths by county, year, & mmwr week (2010-2020) - v6 .xlsx" , 
				sheet("pop- st & race") firstrow allstring ;
		* drop total row ;
			drop if PopulationYearYear == "Total" ;
		* drop unneeded vars ;
			drop SORT ;
		* limit to race categories of interst ;
			keep if RacePopRaceDesc == "Black" | RacePopRaceDesc == "White" ;
		* rename vars ;
			rename 
				(PopulationYearYear Count     RacePopRaceDesc )
				(year_cal           pop_total race ) ;
		* destring pop var ;
		...
		* approximate 2019 & 2020 pop by using 2013-2018 compound annual growth rate ;
			...
			* create vars that equal the 2013 & 2018 values ;
				foreach Y in 2013 2018
					...
					by race: egen pop_total_`Y' = mean(pop_total_`Y'_temp) ;
					} ;
		...
		* inspect var values ;
			* pop ;
				...
				* by race ;
					gsort year_cal ;
					tabstat pop_total, stat(sum) format(%9.0fc) by(race) ;
		* revise race values to lower case ;
			replace race = "black" if race == "Black" ;
			replace race = "white" if race == "White" ;
		* save data ;
...
* merge datasets ;
	* open deaths dataset ;
		use   "`deaths'", clear ;
	* merge pop dataset ;
		merge m:1 year_cal race using "`pop'" ;
		drop _merge ;
* revise & create vars ;
...
* save complete dataset ;
	gsort year_cal week_mmwr race ;
	save "C:\Users\Troy\Box Sync\_ troy's files\research\covid-19\2020-05 - ohio analysis\stata work\covid-19 ohio state all causes by race analysis - sample data - v13 .dta", replace ;
log close ;
end ;
