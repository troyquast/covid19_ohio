...
log using "C:\Users\Troy\Box Sync\_ troy's files\research\covid-19\2020-05 - ohio analysis\stata work\covid-19 ohio state all causes by gender analysis - 01 - create sample data - v13 .smcl", replace ;
* read in, revise, & save datasets ;
	* deaths data ;
		* read in data ;
			import excel 
				...
				sheet("dths- st all gender") firstrow allstring ;
		* drop total row ;
			drop if DeathYearYear == "Total" ;
			drop if DeathSexSexDesc == "Total" ;
		...
		* rename vars ;
			rename 
				(DeathSexSexDesc DeathYearYear MMWRWeekMMWRWeek Deaths     )
				(gender          year_cal      week_mmwr        num_deaths ) ;
		...
		* inspect var values ;
			* # deaths ;
				...
				* by gender ;
					gsort year_cal ;
					tabstat num_deaths, stat(sum) format(%9.0fc) by(gender) ;
					drop if gender == "Unknown" ;
					tabstat num_deaths, stat(sum) format(%9.0fc) by(gender) ;
		* revise gender values to lower case ;
	...
	* pop data ;
		* read in data ;
			import excel 
				"C:\Users\Troy\Box Sync\_ troy's files\research\_data\ohio\public health info warehouse\ohio deaths by county, year, & mmwr week (2010-2020) - v6 .xlsx" , 
				sheet("pop- st & gend") firstrow allstring ;
		* drop total row ;
		...
		* rename vars ;
			rename 
				(PopulationYearYear Count     SexSexDesc )
				(year_cal           pop_total gender ) ;
		* destring pop var ;
...
* merge datasets ;
	* open deaths dataset ;
		use   "`deaths'", clear ;
	* merge pop dataset ;
		merge m:1 year_cal gender using "`pop'" ;
		drop _merge ;
* revise & create vars ;
...
* save complete dataset ;
	gsort year_cal week_mmwr gender ;
	save "C:\Users\Troy\Box Sync\_ troy's files\research\covid-19\2020-05 - ohio analysis\stata work\covid-19 ohio state all causes by gender analysis - sample data - v13 .dta", replace ;
log close ;
end ;
