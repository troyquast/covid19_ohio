...
#delimit ;
log using "C:\Users\Troy\Box Sync\_ troy's files\research\covid-19\2020-05 - ohio analysis\stata work\covid-19 ohio state all causes by age2 (dash) analysis - 02 - estimation - v13 .smcl", replace ;

* open sample data ;
	use "C:\Users\Troy\Box Sync\_ troy's files\research\covid-19\2020-05 - ohio analysis\stata work\covid-19 ohio state all causes by age2 (dash) analysis - sample data - v13 .dta", clear ;
	tempfile sample_data ;
	save   "`sample_data'" ;
	clear ;
* estimate counterfactual mortality ;
	* using fourier terms ;
		foreach A in 00_19 20_49 /* 20_29 30_39 40_49 */ 50_59 60_69 70_79 80_99 
			{ ;
		...
		* append age_group-week files ;
			...
			* append observations for other age groups ;
				foreach A in 20_49 /* 20_29 30_39 40_49 */ 50_59 60_69 70_79 80_99
				{ ;
					foreach W of numlist 1/40
					{ ;
					append using "``A'_`W''" ;
...
* merge w/ full dataset ;
	merge 1:1 year_cal week_mmwr age_group_dash using "C:\Users\Troy\Box Sync\_ troy's files\research\covid-19\2020-05 - ohio analysis\stata work\covid-19 ohio state all causes by age2 (dash) analysis - sample data - v13 .dta" ;
* prep & save data ;
	...
	* rename vars ;
		foreach A in 00_19 20_49 /* 20_29 30_39 40_49 */ 50_59 60_69 70_79 80_99
			{ ;
			rename num_deaths`A' st_allc_`A'_actual ;
			foreach P in 005 025 050 500 950 975 995 
				{ ;
				rename pctile_`P'`A' st_allc_`A'_p`P' ;
				} ;
			} ;
	* order & sort data ;
	...
	* save data ;
		save "C:\Users\Troy\Box Sync\_ troy's files\research\covid-19\2020-05 - ohio analysis\stata work\counter mort datasets\covid-19 ohio state all causes by age2 (dash) analysis - actual & counter counts - v13 .dta", replace ;
log close ;
end ;